...
public class ProductoController {
    ...
    public ResponseEntity<Resource> getImage(@PathVariable String imageName) {
        try {
            System.out.println("🔍 Buscando imagen: " + imageName);

            // Buscar en static/images/
            Resource resource = new ClassPathResource("static/images/" + imageName);

            if (resource.exists()) {
                System.out.println("✅ Imagen encontrada: " + imageName);
                ...
                System.out.println("❌ Imagen NO encontrada: static/images/" + imageName);

                // Intentar buscar en la raíz de static/ por si acaso
                Resource fallbackResource = new ClassPathResource("static/" + imageName);
                if (fallbackResource.exists()) {
                    System.out.println("✅ Imagen encontrada en ubicación alternativa: " + imageName);
                    return ResponseEntity.ok()
            ...
            System.out.println("❌ Error cargando imagen: " + imageName + " - " + e.getMessage());
            return ResponseEntity.notFound().build();
        }
    }

    // ✅ Endpoint para retornar los productos en formato JSON - CON CORS
    @GetMapping("/api")
    @ResponseBody
    ...
    public List<Producto> listarProductosApi() {
        try {
            ...
            System.out.println("✅ Productos encontrados en BD: " + productos.size());

            // Log para debug
            productos.forEach(p -> {
                System.out.println("📦 Producto: " + p.getNombre() + " | Imagen: " + p.getImagen());
            });

            return productos;
        } catch (Exception e) {
            System.err.println("❌ Error en /productos/api: " + e.getMessage());
            e.printStackTrace();
            throw e;
        }
    }

    // ✅ Endpoint adicional para productos destacados
